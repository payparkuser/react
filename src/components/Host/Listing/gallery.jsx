...
class HostGallery extends Component {
  ...
  getGalleryApiCall = spaceId => {
    api
      .postMethod("spaces_galleries", { space_id: spaceId })
      .then(response => {
        if (response.data.success === true) {
          this.setState({
            loading: false,
            gallery: response.data.data.galleries,
            space_details: response.data.data.space_details
          });
        } else {
  ...
  componentWillReceiveProps(nextProps) {
    this.getGalleryApiCall(this.state.spaceId);
  }
  ...
  uploadImages = event => {
    ...
    api
      .postMethod("spaces_upload_files", {
  ...
  deleteImage = (event, space_gallery_id) => {
    event.preventDefault();
    api
      .postMethod("spaces_remove_files", {
        space_id: this.props.match.params.id,
        space_gallery_id: space_gallery_id
      })
      .then(response => {
        if (response.data.success === true) {
          ToastDemo(this.props.toastManager, response.data.message, "success");
                          ...
                          <div className="media user-profile-sec">
                            <div className="media-body">
                              <h2>
                                {loading
                                  ? "Loading..."
                                  : this.state.space_details
                                  ? this.state.space_details.space_name
                                  : ""}
                              </h2>
                              <h4>
